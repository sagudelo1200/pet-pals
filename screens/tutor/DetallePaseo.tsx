...
export const DetallePaseo = () => {
  ...
  const { estado, puede, transicion, cargando: cargandoMaquina, sincronizar } = useEstadoPaseo(paseo || undefined)
    ...
    if (evento === 'CANCELAR') {
      Alert.alert(
         ...
         [
           ...
           { text: 'Sí, Cancelar', style: 'destructive', onPress: () => ejecutar(evento, { motivo: 'Usuario canceló' }) }
         ]
      )
      return
    }
    ...
    <Screen style={styles.container} includeTopInset>
        ...
        <ScrollView contentContainerStyle={styles.content}>
            <Card style={styles.card}>
                ...
                <Text style={styles.info}>Inicio: {paseo.fecha_hora_inicio ? new Date(paseo.fecha_hora_inicio).toLocaleString() : 'N/A'}</Text>
            </Card>
            ...
            <View style={styles.actions}>
                ...
                {/* Botones para probar flujo completo (Simulando ser cuidador/sistema) */}
                 {puede('ACEPTAR') && (
                    <Button title="[Demo] Aceptar" onPress={() => handleTransicion('ACEPTAR')} style={styles.actionBtn} />
                )}
                 {puede('INICIAR_RUTA') && (
                    <Button title="[Demo] Iniciar Ruta" onPress={() => handleTransicion('INICIAR_RUTA')} style={styles.actionBtn} />
                )}
                 {puede('LLEGAR') && (
                    <Button title="[Demo] Llegar" onPress={() => handleTransicion('LLEGAR')} style={styles.actionBtn} />
                )}
                 {puede('INICIAR_PASEO') && (
                    <Button title="[Demo] Iniciar Paseo" onPress={() => handleTransicion('INICIAR_PASEO')} style={styles.actionBtn} variant="primario" />
                )}
                 {puede('FINALIZAR_PASEO') && (
                    <Button title="[Demo] Finalizar" onPress={() => handleTransicion('FINALIZAR_PASEO')} style={styles.actionBtn} />
                )}
                 {puede('CONFIRMAR_COMPLETADO') && (
                    <Button title="Confirmar Completado" onPress={() => handleTransicion('CONFIRMAR_COMPLETADO')} style={styles.actionBtn} variant="exito" />
                )}
...
const styles = StyleSheet.create({
    ...
    backButton: { width: 50, paddingHorizontal: 0, minWidth: 40 },
