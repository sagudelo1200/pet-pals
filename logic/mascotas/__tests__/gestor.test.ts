...
describe('GestorMascotas', () => {
  ...
  test('crear delega a ServicioMascota.crear cuando es válido', async () => {
    ...
    void (ServicioMascota.crear as jest.Mock).mockResolvedValue(mockRes)
  ...
  test('obtenerPorUsuario delega', async () => {
    ...
    void (ServicioMascota.obtenerPorUsuario as jest.Mock).mockResolvedValue(
      mockRes
    )
  ...
  test('obtenerPorId delega', async () => {
    const mockRes = { success: true, data: { id: 'm1' } }
    void (ServicioMascota.obtenerPorId as jest.Mock).mockResolvedValue(mockRes)
  ...
  test('actualizar delega', async () => {
    ...
    void (ServicioMascota.actualizar as jest.Mock).mockResolvedValue(mockRes)
  ...
  test('eliminar delega', async () => {
    const mockRes = { success: true }
    void (ServicioMascota.eliminar as jest.Mock).mockResolvedValue(mockRes)
