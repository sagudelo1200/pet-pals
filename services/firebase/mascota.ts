...
export class MascotaService {
  ...
  static async create(data: Omit<Mascota, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'updatedBy'>): Promise<CrudResult<Mascota>> {
  ...
  static async update(id: string, data: Partial<Omit<Mascota, 'id' | 'createdAt' | 'createdBy'>>): Promise<CrudResult<Mascota>> {
  ...
  static async getByUsuario(userId: string): Promise<CrudResult<Mascota[]>> {
    return BaseCrudService.getWhere<Mascota>(this.COLLECTION, 'id_usuario', userId);
  }
