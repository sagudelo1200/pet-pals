...
export function useEstadoPaseo(paseoInicial?: Partial<Paseo>) {
  ...
  const maquinaRef = useRef<MaquinaEstadosPaseo>(crearMaquinaPaseo(paseoInicial))
   ...
   */
  const transicion = useCallback(async (evento: PaseoEvent, payload?: TransitionPayload) => {
    try {
      setError(null)
      ...
      if (paseoInicial?.id) {
        setCargando(true)
        const res = await ServicioPaseo.actualizarEstado(paseoInicial.id, nuevoEstado, payload as any)
        ...
        if (!res.success) {
          ...
          maquinaRef.current = crearMaquinaPaseo({ ...paseoInicial, estado: anterior })
          setEstado(anterior)
          return false
        }
      }
